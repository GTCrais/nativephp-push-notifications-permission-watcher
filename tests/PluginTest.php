...
/**
 * Plugin validation tests for PushNotificationsPermissionWatcher.
 *
...
describe('Plugin Manifest', function () {
    ...
    it('has required fields', function () {
        ...
        expect($manifest['name'])->toBe('gtcrais/nativephp-push-notifications-permission-watcher');
        expect($manifest['namespace'])->toBe('PushNotificationsPermissionWatcher');
    });
...
describe('Native Code', function () {
    it('has Android Kotlin file', function () {
        $kotlinFile = $this->pluginPath . '/resources/android/PushNotificationsPermissionWatcherFunctions.kt';

        expect(file_exists($kotlinFile))->toBeTrue();

        $content = file_get_contents($kotlinFile);
        expect($content)->toContain('package com.gtcrais.native.push_notifications_permission_watcher');
        expect($content)->toContain('object PushNotificationsPermissionWatcherFunctions');
        expect($content)->toContain('BridgeFunction');
    });

    it('has iOS Swift file', function () {
        $swiftFile = $this->pluginPath . '/resources/ios/PushNotificationsPermissionWatcherFunctions.swift';

        expect(file_exists($swiftFile))->toBeTrue();

        $content = file_get_contents($swiftFile);
        expect($content)->toContain('enum PushNotificationsPermissionWatcherFunctions');
        expect($content)->toContain('BridgeFunction');
    });
...
describe('PHP Classes', function () {
    it('has service provider', function () {
        $file = $this->pluginPath . '/src/PushNotificationsPermissionWatcherServiceProvider.php';
    ...
    it('has facade', function () {
        $file = $this->pluginPath . '/src/Facades/PushNotificationsPermissionWatcher.php';
        expect(file_exists($file))->toBeTrue();
    ...
    it('has main implementation class', function () {
        $file = $this->pluginPath . '/src/PushNotificationsPermissionWatcher.php';
        expect(file_exists($file))->toBeTrue();
